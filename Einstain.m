...
addpath TensorRing
addpath tensorlab
filename = 'Result_Einstain'; 
mkdir(filename);
%% Original Data
load('Einstein.mat');
figure()
imshow(Data);
saveas(gcf, strcat(filename,'/Einstain_FullObserve.pdf'));
Data = double(Data);
...
saveas(gcf, strcat(filename,['/Einstain_ObservingRate' num2str(ObserveRatio) '.pdf']));
...
saveas(gcf, strcat(filename,['/Einstain_Completion(TR)Rank' num2str(r) 'Err' num2str(norm(T2V(Data_Recover_TR-Data))/norm(T2V(Data))) '.pdf']));